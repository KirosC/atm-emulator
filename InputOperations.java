...
public class InputOperations extends JPanel {
  ...
  public JFrame mainFrame(JButton enter, JButton cancel, String[] displayLine, JButton[] l,
      JButton[] r) {
    inputOp = new JFrame("the ATM");
    inputOp.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
    inputOp.setLayout(new BorderLayout(2, 2));
    leftBtns = sideButton(l, true);
    rightBtns = sideButton(r, false);
    inputOp.add(leftBtns, BorderLayout.WEST);
    inputOp.add(rightBtns, BorderLayout.EAST);

    screen = displayScreen(displayLine, false, false, false);
    inputOp.add(screen, BorderLayout.CENTER);
    ent = enter;
    ...
    inputOp.setSize((int) (screenWidth / 3), (int) (screenHeight * 0.8));
    inputOp.setLocationRelativeTo(null);
    inputOp.setVisible(true);
    inputOp.setResizable(false);

    // Set a panel to reserve the size for the keypad, the size is one-third of the main frame
    JPanel keypadPanel = new JPanel();
    keypadPanel.setPreferredSize(new Dimension(inputOp.getWidth(), inputOp.getHeight() / 3));
    keypadPanel.setLayout(new BorderLayout());
    keypadPanel.add(keypad(ent, cancel), BorderLayout.CENTER);
    inputOp.add(keypadPanel, BorderLayout.SOUTH);
  ...
  public JPanel mainMenu(int accountNo) {
    stepCounter = 4;
    String[] displayLine = {"Welcome! " + accountNo, "Please choose a service.",
        " - VIEW MY BALANCE       ",
      ...
      try {
        switch (btn.getText()) { // get button text
          ...
          case ".":
            ...
            if (textField != null) {
              if (value.equals(".")
                  && textField.getText().indexOf(".") != -1) { // more than one decimal point
